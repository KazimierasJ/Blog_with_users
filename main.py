...
def register():
    ...
    if form.validate_on_submit():
        try:
            ...
            user_already_in_db = db.session.query(User).filter_by(email=new_user.email).first() is not None
            ...
            return jsonify(response={"Error": "Failed to register a new user."})
        return redirect(url_for('get_all_posts'))
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            flash('No user is registered with this email address. Please try again.')
            return redirect(url_for('login'))
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            return redirect(url_for('login'))
        # requested_post = BlogPost.query.get(post_id) # TODO delete
        # return render_template("post.html", post=requested_post, form=form) # TODO delete
    # GET
